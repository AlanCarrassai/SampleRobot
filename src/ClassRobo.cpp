...
{
    // Variável para armazenar o ângulo com o sinal correto
    double anguloComSinal = degrees; 

    // O Sentido 2 (Direita) deve ser um ângulo negativo para o ARIA/setDeltaHeading
    if (Sentido == 2) 
    {
        anguloComSinal = -degrees;
    }

    // Aplica o comando de rotação com o sinal corrigido
    robot.setDeltaHeading(anguloComSinal); 

    // O restante da lógica de movimento (avançar/recuar) permanece a mesma
    if (Sentido == 0)
        robot.setVel(0);
    else if (Sentido == 1)
